...
exports.buildInvoiceLineItems = buildInvoiceLineItems;
const calculatePay_1 = require("./calculatePay");
function getWeekStartDate(dateStr, weekStartDay) {
...
function buildInvoiceLineItems(entries, positionsById, rules) {
    ...
    for (const weekKey of sortedWeeks) {
        ...
        const lumpSums = weekEntries.filter((e) => e.type === 'lump_sum');
        ...
        for (const shift of shifts) {
            ...
            if (!position)
                continue;
            const shiftMinutes = (0, calculatePay_1.calcShiftMinutes)(shift.startTime, shift.endTime, shift.mealBreaks);
            const shiftHours = shiftMinutes / 60;
            const { regularHours, overtimeHours } = (0, calculatePay_1.splitShiftHoursForOT)(shiftHours, runningHours, rules);
            runningHours += shiftHours;
            const mealPenalty = (0, calculatePay_1.calcMealPenalty)(shift.startTime, shift.endTime, shift.mealBreaks, position.baseRate, rules);
