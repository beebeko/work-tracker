...
exports.calcShiftMinutes = calcShiftMinutes;
exports.calcLongestStretchHours = calcLongestStretchHours;
exports.calcMealPenalty = calcMealPenalty;
exports.splitShiftHoursForOT = splitShiftHoursForOT;
exports.calculateWeeklyPay = calculateWeeklyPay;
function parseTimeToMinutes(time) {
    const [hours, minutes] = time.split(':').map(Number);
    return hours * 60 + minutes;
}
function calcShiftMinutes(startTime, endTime, mealBreaks) {
...
function calcLongestStretchHours(startTime, endTime, mealBreaks) {
    ...
    const sortedBreaks = [...mealBreaks].sort((a, b) => parseTimeToMinutes(a.startTime) - parseTimeToMinutes(b.startTime));
    let maxStretch = 0;
    ...
    if (finalStretch > maxStretch)
        maxStretch = finalStretch;
    return maxStretch / 60;
}
function calcMealPenalty(startTime, endTime, mealBreaks, baseRate, rules) {
    if (!rules.mealPenaltyEnabled)
        return 0;
...
function splitShiftHoursForOT(shiftHours, runningTotalHours, rules) {
...
function calculateWeeklyPay(entries, rules) {
    ...
    const lumpSums = entries.filter((e) => e.type === 'lump_sum');
    let runningHours = 0;
    let regularHours = 0;
    let regularPay = 0;
    let overtimeHours = 0;
    ...
    for (const shift of shifts) {
        ...
        const { regularHours: reg, overtimeHours: ot } = splitShiftHoursForOT(shiftHours, runningHours, rules);
        regularHours += reg;
        regularPay += reg * shift.baseRate;
        overtimeHours += ot;
        overtimePay += ot * shift.baseRate * rules.weeklyOvertimeMultiplier;
        runningHours += shiftHours;
        mealPenalties += calcMealPenalty(shift.startTime, shift.endTime, shift.mealBreaks, shift.baseRate, rules);
    }
