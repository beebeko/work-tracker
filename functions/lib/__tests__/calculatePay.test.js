 ...
 */
const calculatePay_1 = require("../pay/calculatePay");
const defaultRules = {
...
describe('parseTimeToMinutes', () => {
    it('converts HH:MM to total minutes', () => {
        expect((0, calculatePay_1.parseTimeToMinutes)('08:00')).toBe(480);
        expect((0, calculatePay_1.parseTimeToMinutes)('00:00')).toBe(0);
        expect((0, calculatePay_1.parseTimeToMinutes)('23:59')).toBe(1439);
        expect((0, calculatePay_1.parseTimeToMinutes)('12:30')).toBe(750);
    });
});
// ─── calcShiftMinutes ─────────────────────────────────────────────────────────
describe('calcShiftMinutes', () => {
    it('calculates shift duration with no breaks', () => {
        expect((0, calculatePay_1.calcShiftMinutes)('08:00', '16:00', [])).toBe(480);
    });
    it('subtracts meal break duration', () => {
        const breaks = [{ startTime: '12:00', endTime: '12:30' }];
        expect((0, calculatePay_1.calcShiftMinutes)('08:00', '16:00', breaks)).toBe(450);
    });
    it('handles overnight shifts', () => {
        // 22:00 → 06:00 = 8h
        expect((0, calculatePay_1.calcShiftMinutes)('22:00', '06:00', [])).toBe(480);
    });
    it('handles overnight meal break', () => {
        // 22:00 → 06:00 with a 30-min break at 00:00
        const breaks = [{ startTime: '00:00', endTime: '00:30' }];
        expect((0, calculatePay_1.calcShiftMinutes)('22:00', '06:00', breaks)).toBe(450);
    });
    it('subtracts multiple breaks', () => {
        ...
        expect((0, calculatePay_1.calcShiftMinutes)('08:00', '17:00', breaks)).toBe(495);
    });
});
// ─── calculateWeeklyPay ───────────────────────────────────────────────────────
describe('calculateWeeklyPay', () => {
    it('returns zero pay for empty entries', () => {
        const result = (0, calculatePay_1.calculateWeeklyPay)([], defaultRules);
    ...
    it('calculates straight-time pay under threshold', () => {
        // 8h shift, no OT
        const entries = [
            { type: 'shift', date: '2026-05-04', startTime: '09:00', endTime: '17:00', mealBreaks: [], baseRate: 50 },
        ];
        const result = (0, calculatePay_1.calculateWeeklyPay)(entries, defaultRules);
    ...
    it('calculates weekly overtime beyond threshold', () => {
        ...
        const entries = ['2026-05-04', '2026-05-05', '2026-05-06', '2026-05-07', '2026-05-08'].map((date) => ({
            type: 'shift',
            date,
            startTime: '09:00',
            endTime: '18:00',
    ...
    it('accumulates lump-sum entries', () => {
        const entries = [
            { type: 'lump_sum', amount: 250 },
            { type: 'lump_sum', amount: 100 },
        ];
        const result = (0, calculatePay_1.calculateWeeklyPay)(entries, defaultRules);
    ...
    it('applies meal penalties when enabled', () => {
        ...
        const entries = [
            { type: 'shift', date: '2026-05-04', startTime: '09:00', endTime: '18:00', mealBreaks: [], baseRate: 50 },
        ];
        const result = (0, calculatePay_1.calculateWeeklyPay)(entries, rulesWithPenalty);
    ...
    it('does not include entries sorted later from OT accumulation', () => {
        ...
        const entries = [
            { type: 'shift', date: '2026-05-04', startTime: '09:00', endTime: '17:00', mealBreaks: [], baseRate: 50 },
            { type: 'shift', date: '2026-05-04', startTime: '07:00', endTime: '09:00', mealBreaks: [], baseRate: 50 },
        ];
        const result = (0, calculatePay_1.calculateWeeklyPay)(entries, defaultRules);
    ...
    it('uses splitShiftHoursForOT to correctly split a shift straddling the OT threshold', () => {
        ...
        const result = (0, calculatePay_1.calculateWeeklyPay)([...priorShifts, pushingShift], defaultRules);
        expect(result.regularHours).toBe(40);
