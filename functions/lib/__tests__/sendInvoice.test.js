...
const sendInvoice_1 = require("../sendInvoice");
const invoiceData = {
...
describe('sendInvoice', () => {
    it('is defined', () => {
        expect(sendInvoice_1.sendInvoice).toBeDefined();
        expect(capturedHandler).not.toBeNull();
    });
    it('throws unauthenticated if no auth', async () => {
        await expect(capturedHandler({ auth: null, data: { invoiceId: 'i1', fromAddress: 'a', toAddress: 'b' } })).rejects.toMatchObject({ code: 'unauthenticated' });
    });
    it('throws invalid-argument if required fields missing', async () => {
        await expect(capturedHandler(makeRequest({ invoiceId: 'i1' }))).rejects.toMatchObject({ code: 'invalid-argument' });
    });
    it('throws not-found if invoice does not exist', async () => {
        ...
        await expect(capturedHandler(makeRequest({ invoiceId: 'bad', fromAddress: 'a@x.com', toAddress: 'b@x.com' }))).rejects.toMatchObject({ code: 'not-found' });
    });
    it('throws failed-precondition if invoice has no pdfStoragePath', async () => {
        setupMocks({ pdfStoragePath: null });
        await expect(capturedHandler(makeRequest({ invoiceId: 'invoice-1', fromAddress: 'a@x.com', toAddress: 'b@x.com' }))).rejects.toMatchObject({ code: 'failed-precondition' });
    });
    it('sends email and updates status to sent on success', async () => {
        const { invoiceDoc } = setupMocks();
        const result = await capturedHandler(makeRequest({ invoiceId: 'invoice-1', fromAddress: 'a@x.com', toAddress: 'b@x.com' }));
        expect(mockSendFn).toHaveBeenCalled();
        expect(invoiceDoc.update).toHaveBeenCalledWith(expect.objectContaining({ status: 'sent' }));
        expect(result).toEqual({ success: true });
    });
    it('throws internal on Resend failure', async () => {
        setupMocks();
        mockSendFn.mockResolvedValueOnce({ error: { message: 'Rate limited' }, data: null });
        await expect(capturedHandler(makeRequest({ invoiceId: 'invoice-1', fromAddress: 'a@x.com', toAddress: 'b@x.com' }))).rejects.toMatchObject({ code: 'internal' });
    });
    it('throws permission-denied when invoice belongs to another user', async () => {
        setupMocks({ ownerUid: 'other-uid' });
        await expect(capturedHandler(makeRequest({ invoiceId: 'invoice-1', fromAddress: 'a@x.com', toAddress: 'b@x.com' }, 'test-uid'))).rejects.toMatchObject({ code: 'permission-denied' });
    });
});
