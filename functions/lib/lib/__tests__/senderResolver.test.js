...
const senderResolver_1 = require("../senderResolver");
beforeEach(() => {
    jest.clearAllMocks();
...
describe('resolveSenderToClient', () => {
    describe('happy path', () => {
        it('resolves an exact address match', async () => {
            ...
            const result = await (0, senderResolver_1.resolveSenderToClient)('scheduler@acme.com');
        ...
        it('resolves a domain wildcard match when no address match exists', async () => {
            ...
            const result = await (0, senderResolver_1.resolveSenderToClient)('anyone@acme.com');
    ...
    describe('error handling', () => {
        it('returns null when no sender matches', async () => {
            ...
            const result = await (0, senderResolver_1.resolveSenderToClient)('unknown@stranger.com');
            expect(result).toBeNull();
        });
        it('propagates Firestore errors', async () => {
            mockDb.get.mockRejectedValue(new Error('Firestore down'));
            await expect((0, senderResolver_1.resolveSenderToClient)('scheduler@acme.com')).rejects.toThrow('Firestore down');
        });
    });
    describe('bad input', () => {
        it('returns null for an address with no domain part', async () => {
            mockDb.get.mockResolvedValue(emptySnap);
            const result = await (0, senderResolver_1.resolveSenderToClient)('nodomain');
            expect(result).toBeNull();
        });
    });
});
