import { extractAddress, ResendAdapter, SendGridAdapter } from '../lib/emailIngestion';

describe('emailIngestion', () => {
  describe('extractAddress', () => {
    it('extracts bare address from "Name <addr>" header', () => {
      expect(extractAddress('Scheduler <scheduler@acme.com>')).toBe('scheduler@acme.com');
    });

    it('returns lowercased trimmed address when header has no name', () => {
      expect(extractAddress('  Scheduler@Acme.com  ')).toBe('scheduler@acme.com');
    });

    it('lowercases addresses inside angle brackets', () => {
      expect(extractAddress('"Name" <SCHEDULER@ACME.COM>')).toBe('scheduler@acme.com');
    });
  });
  ...
  describe('ResendAdapter', () => {
    ...
    it('throws when from is an empty object', () => {
      expect(() => adapter.parse({ data: { from: {}, text: 'body' } })).toThrow(
