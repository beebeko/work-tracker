...
function makeSnap(data: object, exists = true) {
...
function setupMocks() {
  ...
  mockDb.runTransaction = jest.fn().mockImplementation(async (cb: (tx: any) => Promise<string>) => {
  ...
  const invoicesCollection = {
    doc: jest.fn().mockImplementation((id?: string) => {
      if (id === 'existing-invoice-id') return existingInvoiceDoc;
      return newInvoiceRef;
    }),
  };

  mockDb.collection.mockImplementation((name: string) => {
    if (name === 'userProfiles') return { doc: jest.fn().mockReturnValue(profileDoc) };
    if (name === 'clients') return { doc: jest.fn().mockReturnValue(clientDoc) };
    if (name === 'gigs') return { doc: jest.fn().mockReturnValue(gigDoc) };
    if (name === 'workEntries') return { doc: jest.fn().mockReturnValue(entryDoc) };
    if (name === 'positions') return { doc: jest.fn().mockReturnValue(positionDoc) };
...
describe('generateInvoice', () => {
  ...
  it('throws unauthenticated if no auth', async () => {
    await expect(
      capturedHandler!({ auth: null, data: { clientId: 'c1', gigId: 'g1', entryIds: [] } }),
    ).rejects.toMatchObject({ code: 'unauthenticated' });
  });

  it('throws invalid-argument if required fields missing', async () => {
    await expect(
      capturedHandler!(makeRequest({ gigId: 'g1', entryIds: [] })),
  ...
  it('throws not-found if client does not exist', async () => {
    ...
    mockDb.collection.mockImplementation((name: string) => {
      if (name === 'clients') return { doc: jest.fn().mockReturnValue({ get: jest.fn().mockResolvedValue({ exists: false }) }) };
      if (name === 'gigs') return { doc: jest.fn().mockReturnValue({ get: jest.fn().mockResolvedValue({ exists: true, data: () => gigData, id: 'gig-1' }) }) };
      if (name === 'userProfiles') return { doc: jest.fn().mockReturnValue({ get: jest.fn().mockResolvedValue({ exists: false }) }) };
      return { doc: jest.fn() };
    });

    await expect(
      capturedHandler!(makeRequest({ clientId: 'bad', gigId: 'g1', entryIds: [] })),
  ...
  it('creates a new invoice and returns invoiceId', async () => {
    ...
    const result = await capturedHandler!(
      makeRequest({ clientId: 'client-1', gigId: 'gig-1', entryIds: ['entry-1'] }),
    );
    ...
    expect(tx.update).toHaveBeenCalledWith(
      expect.anything(),
      expect.objectContaining({ nextInvoiceSeq: expect.anything() }),
    );
  });
  ...
  it('throws permission-denied if client belongs to a different user', async () => {
    ...
    mockDb.collection.mockImplementation((name: string) => {
      if (name === 'clients') return { doc: jest.fn().mockReturnValue(otherClientDoc) };
      if (name === 'gigs') return { doc: jest.fn().mockReturnValue({ get: jest.fn().mockResolvedValue({ exists: true, data: () => ({ ...gigData, ownerUid: 'test-uid' }), id: 'gig-1' }) }) };
      if (name === 'userProfiles') return { doc: jest.fn().mockReturnValue({ get: jest.fn().mockResolvedValue({ exists: false }) }) };
      return { doc: jest.fn() };
    });

    await expect(
      capturedHandler!(makeRequest({ clientId: 'client-1', gigId: 'gig-1', entryIds: [] })),
  ...
  it('throws permission-denied if gig belongs to a different user', async () => {
    ...
    mockDb.collection.mockImplementation((name: string) => {
      if (name === 'clients') return { doc: jest.fn().mockReturnValue(clientDoc) };
      if (name === 'gigs') return { doc: jest.fn().mockReturnValue(otherGigDoc) };
      if (name === 'userProfiles') return { doc: jest.fn().mockReturnValue({ get: jest.fn().mockResolvedValue({ exists: false }) }) };
      return { doc: jest.fn() };
    });
  ...
  it('regenerates an existing invoice and archives history', async () => {
    ...
    const result = await capturedHandler!(
      makeRequest({ clientId: 'client-1', gigId: 'gig-1', entryIds: ['entry-1'], invoiceId: 'existing-invoice-id' }),
    );

    expect(result).toEqual({ invoiceId: 'existing-invoice-id' });
    expect(existingInvoiceDoc.update).toHaveBeenCalledWith(
  ...
  it('throws not-found when regenerating a non-existent invoice', async () => {
    ...
    mockDb.collection.mockImplementation((name: string) => {
      ...
      if (name === 'clients') return { doc: jest.fn().mockReturnValue({ get: jest.fn().mockResolvedValue({ exists: true, data: () => ({ ...clientData, ownerUid: 'test-uid' }), id: 'client-1' }) }) };
      if (name === 'gigs') return { doc: jest.fn().mockReturnValue({ get: jest.fn().mockResolvedValue({ exists: true, data: () => ({ ...gigData, ownerUid: 'test-uid' }), id: 'gig-1' }) }) };
      if (name === 'userProfiles') return { doc: jest.fn().mockReturnValue({ get: jest.fn().mockResolvedValue({ exists: false }) }) };
      if (name === 'workEntries') return { doc: jest.fn().mockReturnValue({ get: jest.fn().mockResolvedValue({ exists: false }) }) };
      return { doc: jest.fn() };
    });

    await expect(
      capturedHandler!(makeRequest({ clientId: 'client-1', gigId: 'gig-1', entryIds: [], invoiceId: 'missing-invoice' })),
    ).rejects.toMatchObject({ code: 'not-found' });
  });
  ...
  it('throws permission-denied when regenerating an invoice owned by another user', async () => {
    ...
    await expect(
      capturedHandler!(makeRequest({ clientId: 'client-1', gigId: 'gig-1', entryIds: [], invoiceId: 'existing-invoice-id' })),
    ).rejects.toMatchObject({ code: 'permission-denied' });
  });
});
