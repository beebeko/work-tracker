...
describe('buildPdf', () => {
  it('returns a non-empty Uint8Array', async () => {
    const bytes = await buildPdf(baseData);
  ...
  it('handles missing optional fields (dueDate, notes, senderAddress, senderPhone)', async () => {
    ...
    await expect(buildPdf(minimal)).resolves.toBeInstanceOf(Uint8Array);
  });

  it('handles empty line items', async () => {
    const noItems: PdfInvoiceData = { ...baseData, lineItems: [], subtotal: 0, totalAmount: 0 };
    await expect(buildPdf(noItems)).resolves.toBeInstanceOf(Uint8Array);
  });

  it('includes notes section when notes provided', async () => {
    const withNotes: PdfInvoiceData = { ...baseData, notes: 'Payment due net 30' };
    const bytes = await buildPdf(withNotes);
    // PDF will be larger with extra content
    const bytesNoNotes = await buildPdf(baseData);
    expect(bytes.length).toBeGreaterThan(bytesNoNotes.length);
  });

  it('renders all optional fields without throwing', async () => {
    const withAllOptionals: PdfInvoiceData = {
      ...baseData,
      dueDate: '2026-06-25',
    ...
    await expect(buildPdf(withAllOptionals)).resolves.toBeInstanceOf(Uint8Array);
  });

  it('omits optional fields when all undefined', async () => {
    const noOptionals: PdfInvoiceData = {
  ...
  it('throws when too many line items cause overflow', async () => {
    ...
    await expect(buildPdf(manyItems)).rejects.toThrow(/too many line items/i);
  });
});
